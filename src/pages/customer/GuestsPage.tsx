...
function ImportGuestsDialog({
  ...
  async function submit(e: React.FormEvent) {
    ...
    toast.success(`Imported ${rows.length} ${rows.length === 1 ? "guest" : "guests"}`);
    setText("");
    onOpenChange(false);
    onSuccess();
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg rounded-sm">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl">Import guests</DialogTitle>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-4 pt-2">
          <div className="space-y-2">
            <Label htmlFor="import-text">Paste CSV or one name per line</Label>
            <Textarea
              id="import-text"
              value={text}
