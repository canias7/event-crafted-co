...
export function MySpaceChat({ docked = false }: { docked?: boolean } = {}) {
  const { user } = useAuth();
  // Which listing My Space works on is resolved server-side: if the vendor has
  // a single listing it's used automatically; if they have several, the
  // assistant asks which one and remembers the choice (set_active_listing).
