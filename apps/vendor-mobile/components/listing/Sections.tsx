...
// ============================================================================
// TEAM BIOS
// vendor_team_bios: display_name, role, bio, photo_storage_path,
// is_owner, display_order. Photo upload skipped on mobile for now —
// editing focuses on name + role + bio. Photos can be added on web.
// ============================================================================
...
export function TeamSection({ vendorId }: { vendorId: string }) {
  const [items, setItems] = useState<TeamRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<TeamDraft | null>(null);
  const [busy, setBusy] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await (supabase as any)
      ...
      .select(
        "id, display_name, role, bio, photo_storage_path, is_owner, display_order",
      )
      .eq("vendor_id", vendorId)
      .order("display_order", { ascending: true })
      .order("created_at", { ascending: true });
  ...
  async function save() {
    if (!editing) return;
    const name = editing.display_name.trim();
    if (!name) {
  ...
  async function remove(id: string) {
    Alert.alert("Remove team member?", "This can't be undone.", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Remove",
        style: "destructive",
  ...
  return (
    <View className="gap-3">
      <SectionHeader
        title="Team bios"
        subtitle="Public team — names, roles, short bios"
        onAdd={() => setEditing({ ...EMPTY_TEAM })}
      />
      {loading ? (
        <Text className="text-xs text-muted-foreground">Loading…</Text>
      ) : items.length === 0 ? (
        <Text className="text-xs text-muted-foreground">
          No team members yet. Add the lead and any frequent collaborators.
        </Text>
      ) : (
        items.map((m) => (
          <View
            ...
            >
              <View>
                ...
                <TextInput
                  ...
                  onChangeText={(v) =>
                    setEditing({ ...editing, display_name: v })
                  }
              ...
              <View className="flex-row items-center justify-between">
                ...
                <Switch
                  value={editing.is_owner}
                  onValueChange={(v) =>
                    setEditing({ ...editing, is_owner: v })
                  }
                />
              </View>
              <Text className="text-xs text-muted-foreground">
                Photo can be added from the website. Mobile editor focuses on
