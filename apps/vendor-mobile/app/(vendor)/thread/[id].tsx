// Conversation screen reached from Inbox row tap. Loads the
// direct_thread by id, shows messages as bubbles (vendor messages
// right, host messages left), and sends new messages on submit.
// Realtime subscription keeps the view fresh while open.
...
export default function ThreadScreen() {
  ...
  const loadHeader = useCallback(async () => {
    ...
    const { data } = await (supabase as any)
      .from("direct_threads")
      .select(
        "id, host_id, inquiry_id, host:profiles!direct_threads_host_id_fkey(display_name), inquiry:inquiries!direct_threads_inquiry_id_fkey(event_type, event_date)",
      )
      .eq("id", threadId)
      .maybeSingle();
    if (!data) return;
    ...
    setHeader({
      hostName: data.host?.display_name ?? "Host",
  ...
  // Realtime: append new messages as they land.
  useEffect(() => {
    if (!threadId) return;
  ...
  async function send() {
    ...
    if (!error) {
      setDraft("");
      // The realtime subscription will push the row, but on slow
      // networks the user expects instant feedback — optimistic append
      // with a temp id keeps the UI snappy.
      ...
      >
        ...
        >
          {loading ? (
            ...
            messages.map((m) => <Bubble key={m.id} msg={m} mine={m.sender_role === "vendor"} />)
          )}
        </ScrollView>
