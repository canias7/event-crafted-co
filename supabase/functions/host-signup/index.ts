// Host signup flow with a 6-digit email code. Bypasses Supabase
// Auth's built-in confirmation email (which would send a magic link
// using the project's GoTrue templates) by handling the code
// generation, send, and user creation ourselves.
//
...
//
// Mirrors signin-2fa for the code-store + email-send pattern.
...
<html><body style="margin:0;background:#f7f5f2;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;color:#1a1a1a;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="padding:48px 16px;"><tr><td align="center">
    <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:8px;padding:40px 32px;">
      <tr><td style="font-size:14px;letter-spacing:0.18em;color:#a08259;text-transform:uppercase;padding-bottom:24px;">— Vendora</td></tr>
      <tr><td style="font-size:24px;line-height:1.25;font-weight:600;padding-bottom:16px;">Confirm your email</td></tr>
      <tr><td style="font-size:15px;line-height:1.6;color:#3a3a3a;">
        <p style="margin:0 0 16px;">Use this 6-digit code to finish creating your Vendora account:</p>
        <p style="margin:0 0 24px;text-align:center;">
  ...
  const r = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM_ADDRESS,
      to: email,
      subject: `Your Vendora confirmation code is ${cleanCode}`,
      html,
    }),
  });
  if (!r.ok) {
...
serve(async (req) => {
  ...
  if (action === "verify") {
    ...
    // Code is valid. Create the auth user with email_confirm:true so
    // they can sign in immediately. handle_new_user picks up the
    // intended_role and provisions the host profile.
