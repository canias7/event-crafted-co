...
serve(async (req) => {
  ...
  try {
    ...
    // Account-wide opt-in gate: the "Automated invoice delivery" toggle
    // governs AUTOMATIC client emails. A manual "Send to" (to_email
    // override) is the vendor deliberately sending this one email, so it
    // bypasses the gate — pausing automation shouldn't block a hand-sent
    // copy. The automated path (no override) still requires the opt-in.
    if (!toOverride) {
