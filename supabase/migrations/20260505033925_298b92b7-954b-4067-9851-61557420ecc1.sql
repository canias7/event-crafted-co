    ...
    and exists (
      ...
      where vp.id::text = (storage.foldername(name))[1]
        and (vp.user_id = auth.uid()
          or exists (select 1 from public.vendor_team_members tm where tm.vendor_id = vp.id and tm.user_id = auth.uid()))
    )
  );
...
begin
  ...
  values (new.host_id, 'album_published', v_vendor_name || ' published your photos',
    'Your "' || new.title || '" album is ready to view.', '/album/' || new.share_token);
  return new;
end$$;
...
declare v_album record; v_vendor_name text; v_photos jsonb;
begin
  select * into v_album from public.event_albums
    where share_token = p_token and published_at is not null and host_consent_given_at is not null;
  if v_album.id is null then return null; end if;
  select coalesce(business_name, 'Vendor') into v_vendor_name from public.vendor_profiles where id = v_album.vendor_id;
  select coalesce(jsonb_agg(jsonb_build_object('id', p.id, 'storage_path', p.storage_path, 'caption', p.caption, 'taken_at', p.taken_at)
    order by p.display_order, p.taken_at, p.created_at), '[]'::jsonb)
    into v_photos from public.event_album_photos p where p.album_id = v_album.id;
  return jsonb_build_object('album', jsonb_build_object('id', v_album.id, 'title', v_album.title, 'description', v_album.description,
    'cover_path', v_album.cover_path, 'download_enabled', v_album.download_enabled, 'published_at', v_album.published_at,
    'vendor_id', v_album.vendor_id, 'vendor_name', v_vendor_name), 'photos', v_photos);
end$$;
...
create policy "event albums published read" on storage.objects for select
  using (bucket_id = 'event-albums' and exists (
    ...
    where p.storage_path = name and a.published_at is not null and a.host_consent_given_at is not null));