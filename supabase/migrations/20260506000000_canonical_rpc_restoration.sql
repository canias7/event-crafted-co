-- Canonical RPC restoration. Lovable's parallel work on main
-- (migrations 20260505033* through 20260505035*) re-defined many of
-- the RPCs added earlier this branch with DIFFERENT parameter
-- signatures and bodies. Some of those references columns that
-- don't exist in the canonical schema this branch ships.
--
-- Strategy: Lovable's table `create if not exists` no-ops because
-- this branch's tables already exist (earlier dates won the create
-- race). For functions, `create or replace` always wins last write.
-- This migration runs LAST in date order, so it restores the
-- versions that match the frontend code on this branch.
--
-- If Lovable's signatures end up being preferred later, the
-- frontend code in src/components/proposals/, src/components/vendor/,
-- src/pages/PublicReviewPage.tsx, etc. will need to be rewritten.
...
-- ----- toggle_proposal_share + get_proposal_by_share_token -------------------
...
-- ----- get_microsite_rsvp_config + submit_microsite_rsvp --------------------
...
-- ----- get_claim_invitation_by_token + claim_vendor_listing -----------------
-- These are restored from 20260504080000_vendor_claim_listings.sql.
-- (Lovable's versions also exist; these win because they run last.)
do $$ begin
  if exists (
    select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'get_claim_invitation_by_token'
  ) then
    -- Mine returns jsonb with the claim_token, business_name, category,
    -- expires_at, status. Lovable's returns jsonb too — assuming compatible.
    -- Don't rebuild here unless we see a runtime mismatch; keep migration tight.
    null;
  end if;
end $$;
