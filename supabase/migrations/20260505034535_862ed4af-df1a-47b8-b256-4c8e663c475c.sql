...
begin
  if new.verified_at is distinct from old.verified_at and public.is_admin() then
    ...
    values (auth.uid(),
      ...
      'vendor_profile', new.id, coalesce(new.business_name, '(unnamed vendor)'),
...
create trigger vendor_profiles_audit after update of verified_at on public.vendor_profiles
...
begin
  if new.status is distinct from old.status and public.is_admin() then
    select business_name into v_business from public.vendor_profiles where id = new.vendor_id;
    insert into public.admin_audit_log (admin_id, action, target_type, target_id, summary, metadata)
    values (auth.uid(), 'verification_' || new.status, 'vendor_verification', new.id,
...
create trigger vendor_verifications_audit after update of status on public.vendor_verifications
...
begin
  if new.hidden_at is distinct from old.hidden_at and public.is_admin() then
    ...
    values (auth.uid(),
      ...
      'review', new.id, substring(coalesce(new.body, '(no body)') for 100),
      jsonb_build_object('reason', new.hidden_reason));
  end if;
  return new;
end$$;
drop trigger if exists reviews_audit on public.reviews;
create trigger reviews_audit after update of hidden_at on public.reviews
...
begin
  if new.status is distinct from old.status and public.is_admin() then
    insert into public.admin_audit_log (admin_id, action, target_type, target_id, summary, metadata)
    values (auth.uid(), 'support_ticket_' || new.status, 'support_ticket', new.id, new.subject,
...
create trigger support_tickets_audit after update of status on public.support_tickets
