...
declare v_board public.mood_boards; v_items jsonb;
begin
  select * into v_board from public.mood_boards where share_token = p_token;
  if v_board.id is null then return null; end if;
  select coalesce(jsonb_agg(jsonb_build_object('id', i.id, 'image_url', i.image_url, 'source_url', i.source_url, 'caption', i.caption, 'display_order', i.display_order) order by i.display_order, i.created_at), '[]'::jsonb)
    into v_items from public.mood_board_items i where i.board_id = v_board.id;
  return jsonb_build_object('id', v_board.id, 'name', v_board.name, 'description', v_board.description, 'cover_image_url', v_board.cover_image_url, 'created_at', v_board.created_at, 'items', v_items);
end$$;
